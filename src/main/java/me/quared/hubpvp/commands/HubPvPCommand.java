...
public class HubPvPCommand implements CommandExecutor, TabCompleter {
	...
	public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, String[] args) {
		if (args.length != 0) {
			...
			if (args[0].equalsIgnoreCase("reload")) {
				...
				plugin.reloadConfig();
				plugin.pvpManager().loadItemsFromConfig();
				...
				ItemStack weapon = HubPvP.instance().pvpManager().getWeapon();
				player.getInventory().addItem(weapon);
				sender.sendMessage(ChatColor.GREEN + "A weapon was given to player " + player.getName());
			}
			else sender.sendMessage(ChatColor.RED + "Invalid arguments. Use: /" + label + " <reload>");
		} else sender.sendMessage(ChatColor.RED + "Invalid usage. Use: /" + label + " <args>");
		return false;
	}
	...
	public List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command cmd, @NotNull String label, String[] args) {
		if (args.length == 1) {
			...
			return commands.stream().filter(s -> s.startsWith(args[0])).collect(Collectors.toList()); // Filter based on what the user has already typed
		}
